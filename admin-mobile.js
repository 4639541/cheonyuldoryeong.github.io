...
async function loadPlus(){
  try{
    ...
    if($("statTodayBookings")) $("statTodayBookings").textContent = bookings.filter(b=>b.date===today).length;
...
setInterval(loadPlus,3000);
setTimeout(loadPlus,1000);
setTimeout(()=>{
