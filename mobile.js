...
// ===== 8.9 home router final =====
window.CY890={home:"home",reserve:"reserve",shop:"shop",reviews:"reviews",support:"supportPage",hist:"bookingHistory",my:"mypage",business:["businessInfoHome"]};
function cy890Target(t){t=(t||'').replace(/\s+/g,' ').trim(); if(t==='홈')return CY890.home; if(t==='예약'||t==='상담 예약하기')return CY890.reserve; if(t==='상품'||t==='상품 보기'||t==='상품 구매하기')return CY890.shop; if(t==='후기')return CY890.reviews; if(t==='문의')return CY890.support; if(t==='예약이력')return CY890.hist; if(t==='마이'||t==='내 정보')return CY890.my; return '';}
function cy890HomeClean(){var h=document.getElementById(CY890.home), hero=document.getElementById('homeHeroFinal890'); if(h&&hero)h.prepend(hero); (CY890.business||[]).forEach(id=>{var b=document.getElementById(id); if(b){b.style.display='none';b.classList.add('hide');b.classList.remove('on','active');}}); if(h)[...h.children].forEach(el=>{if(el.id==='homeHeroFinal890')return; var tx=el.textContent||''; if((tx.includes('사업자 정보')||tx.includes('사업자등록번호')||tx.includes('결제 안내')&&tx.includes('카카오페이'))){el.style.display='none';el.classList.add('cy890HomeHidden');}});}
function cy890Show(id){if(!document.getElementById(id))id=CY890.home; document.querySelectorAll('.page').forEach(p=>{p.style.display='none';p.classList.add('hide');p.classList.remove('on','active');}); var el=document.getElementById(id); if(el){el.style.display='block';el.classList.remove('hide');el.classList.add('on','active');} if(id===CY890.home)cy890HomeClean(); else (CY890.business||[]).forEach(bid=>{var b=document.getElementById(bid); if(b) b.style.display='none';}); var nav=document.getElementById('stableBottomNavFinal'); if(nav)nav.querySelectorAll('button').forEach(b=>b.classList.toggle('on',b.dataset.page===id)); sessionStorage.setItem('cy890page',id); window.scrollTo({top:0,behavior:'smooth'});}
function cy890Bind(){document.querySelectorAll('.bottomNav').forEach(n=>{if(n.id!=='stableBottomNavFinal'){n.style.display='none';n.style.visibility='hidden';n.style.opacity='0';}}); document.querySelectorAll('#stableBottomNavFinal button').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();cy890Show(b.dataset.page);return false;}); document.querySelectorAll('button,a').forEach(el=>{var page=cy890Target(el.textContent||''); if(page){el.removeAttribute('href');el.setAttribute('data-go',page); if((el.textContent||'').trim()==='상품 구매하기')el.textContent='상품 보기'; el.onclick=e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();cy890Show(page);return false;};}});}
document.addEventListener('click',function(e){var el=e.target.closest('button,a'); if(!el)return; var page=cy890Target(el.textContent||''); if(page){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();cy890Show(page);return false;}},true);
window.go=cy890Show; window.addEventListener('load',()=>{cy890Bind();cy890Show(sessionStorage.getItem('cy890page')||CY890.home);}); setInterval(()=>{cy890Bind(); var h=document.getElementById(CY890.home); if(h&&(h.classList.contains('on')||h.classList.contains('active')))cy890HomeClean();},1500);
